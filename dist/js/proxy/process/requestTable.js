 ...
 export const requestTable = (url,columns = [],edit = true , remove = true,deleteUrl,updateUrl) => {
     ...
     if(edit){
        ...
        $(document).on('click','.edit' ,function(){
         ...
         $('#UpdateSubmit input[name=userID]').val($(`#${id} .userID`).text());
         $('#UpdateSubmit input[name=captainID]').val( $(`#${id} .captainID`).text());
         $('#UpdateSubmit input[name=state]').val($(`#${id} .state`).text());
         $('#UpdateSubmit input[name=statu]').val($(`#${id} .statu`).text());
 ...
 export const userData = (data) => {
    return post(`admin/getUsers`,data,true,'application/json').then(res => {
        console.log(res);
        if(res.data.success){
            $("#userID").text(res.data.data[0].fullName);
            $("#phoneNumber").text(res.data.data[0].phoneNumber);
            $("#phoneNumber").attr('href',`tel:${res.data.data[0].phoneNumber}`);
            $("#email").text(res.data.data[0].email);
            $("#email").attr('href',`mailto:${res.data.data[0].email}`);
        }
        return res
    }).catch(err => {
        console.log(err);
        $('.userData').empty().html('<small>حدث خطأ في جلب المعلومات</small>')
    })
}